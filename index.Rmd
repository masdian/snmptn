...
kable(data) %>%
  ...
  footnote(symbol = "Sudah memiliki NIM")
...
### Berikut adalah daftar nama mahasiswa baru Universitas Sultan Ageng Tirtayasa jalur SNMPTN 2020 sampai dengan 20 Mei 2020. (**gunakan Search untuk pencarian cepat**)

```{r}
# `r format(Sys.time(), "%d %B %Y")` pukul `r format(Sys.time(), "%H:%M")`
# format waktu
data <- db %>%
  filter(nim == "diterima") %>%
  select(no_pend, nama, jk, prodi)
